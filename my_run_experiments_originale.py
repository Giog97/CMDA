...
from tools import train_originale
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', type=str, choices=['mit_b3', 'mit_b4', 'mit_b5', 'mit_b2'], default='') #Qui è dove scegliamo la Backbone
    ...
    if args.base_config is not None:
        ...
        if args.image_change_range != -1:
            assert cfg['data']['val']['type'] == 'DSECDataset_originale' #aggiunto '_originale'
        ...
        elif cfg['data']['val']['type'] == 'DSECDataset_originale': # aggiunto '_originale'
            cfg_data = {'samples_per_gpu': cfg['data']['samples_per_gpu'],
    ...
    if args.machine == 'local':
        for i, cfg in enumerate(cfgs):  # 一个程序运行多个cfg
            ...
            train_originale.main([config_files[i]])
            torch.cuda.empty_cache()
    else:
        raise NotImplementedError(args.machine)

    train_command = 'python my_run_experiments_originale.py --base_config configs/fusion/cs2dz_image+raw-isr_b2.py --server_type ecust --name cs2dz_image+raw-isr_newday_shift3_israug_b2 --isr_mix_aug --shift_3_channel --cs_isr_data_type new_day'
    '''
    DSEC SOTA[56.89-60.05]: 
    CUDA_VISIBLE_DEVICES=2 nohup
    python my_run_experiments_originale.py --server_type ren --base_config configs/fusion/cs2dsec_image+events_together_b2_originale.py 
