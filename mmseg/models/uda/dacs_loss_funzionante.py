...
class ProjectionHead(nn.Module):
    def __init__(self, input_dim=512, projection_dim=128): # mit_b2 ha 512 canali
        super().__init__()
        # Usiamo convoluzioni 1x1 per mantenere le dimensioni spaziali
        self.projection = nn.Sequential(
            nn.Conv2d(input_dim, input_dim, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(input_dim, projection_dim, kernel_size=1)
        )
...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, anchor_feat, positive_feat):
        ...
        anchor_flat = anchor_feat.permute(0, 2, 3, 1).reshape(-1, C)
        positive_flat = positive_feat.permute(0, 2, 3, 1).reshape(-1, C)

        sim_matrix = torch.matmul(anchor_flat, positive_flat.T) / self.temperature

        labels = torch.arange(B * H * W, dtype=torch.long, device=sim_matrix.device)
...
class DACS(UDADecoratorFusion):
    ...
    def __init__(self, **cfg):
        ...
        if self.enable_contrastive:
            ...
            backbone_feat_dim = 320 # Dimensione dell'output di mit_b2 (ultimo stadio)
        ...
        if self.enable_contrastive:
                ...
                if 'f_image' in target_feat_dict and 'f_events' in target_feat_dict and \
                   target_feat_dict['f_image'] is not None and target_feat_dict['f_events'] is not None:
                    ...
                    anchor_tensor_list = target_feat_dict['f_image']
                    positive_tensor_list = target_feat_dict['f_events']
                    
                    # Usiamo feature da uno strato intermedio, non l'ultimo, per avere più dettaglio spaziale
                    # Esempio: il secondo o terzo strato (-2 o -3)
                    anchor_tensor = anchor_tensor_list[-2] 
                    positive_tensor = positive_tensor_list[-2]

                    # --- INIZIO MODIFICA DENSE ---
                    # NON facciamo più il pooling. Passiamo le feature 2D alla projection head.
                    anchor_feat_proj = self.projection_head(anchor_tensor)
                    positive_feat_proj = self.projection_head(positive_tensor)

                    # Normalizziamo lungo i canali per ogni pixel
                    anchor_feat_norm = F.normalize(anchor_feat_proj, p=2, dim=1)
                    positive_feat_norm = F.normalize(positive_feat_proj, p=2, dim=1)
                    
                    # Calcoliamo la loss densa passando solo l'ancora e il positivo
                    # La loss stessa gestirà la creazione dei negativi in-batch
                    loss_contrastive = self.contrastive_loss(anchor_feat_norm, positive_feat_norm)
