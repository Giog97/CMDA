...
class DACS(UDADecoratorFusion):
    ...
    def forward_train(self, **kwargs):
        ...
        if self.enable_contrastive:
            ...
            print(f"=== CONTRASTIVE DEBUG Iter {self.local_iter} ===")
            print(f"enable_contrastive: {self.enable_contrastive}")
            ...
            print(f"contrastive_features exists: {'contrastive_features' in pred_dict}")
            print(f"contrastive_features keys: {list(contrastive_features.keys()) if contrastive_features else 'None'}")
            ...
            if self.local_iter % 10 == 0:
                ...
                if 'contrastive_features' in pred_dict:
                    ...
                    if contrastive_features.get('image') is not None:
                        print(f"Image feature shape: {contrastive_features['image'].shape}")
                    if contrastive_features.get('events') is not None:
                        print(f"Events feature shape: {contrastive_features['events'].shape}")
            
            # CONTROLLO ROBUSTO - Versione migliorata
            """
            has_valid_contrastive = False
            if (contrastive_features and 
                ...
                hasattr(contrastive_features['image'], 'shape') and
                hasattr(contrastive_features['events'], 'shape') and
                len(contrastive_features['image'].shape) == 2 and  # [B, D]
                len(contrastive_features['events'].shape) == 2 and  # [B, D]
                contrastive_features['image'].shape[0] > 0 and
                contrastive_features['events'].shape[0] > 0 and
                contrastive_features['image'].shape[0] == contrastive_features['events'].shape[0]):  # Stesso batch size
                
                has_valid_contrastive = True
                anchor_feat = contrastive_features['image']
                positive_feat = contrastive_features['events']
            """
            # CONTROLLO MOLTO PIÙ PERMISSIVO - Solo per debug
            has_valid_contrastive = False
            if (contrastive_features and 
                'image' in contrastive_features and 
                'events' in contrastive_features and
                ...
                # ACCETTA ANCHE FEATURES CON SHAPE [1, D] (batch size 1)
                anchor_feat = contrastive_features['image']
                positive_feat = contrastive_features['events']
                
                has_valid_contrastive = True
                print(f"CONTRASTIVE ATTIVO! Anchor: {anchor_feat.shape}, Positive: {positive_feat.shape}")
                
            if has_valid_contrastive:
                # Debug delle shape
                if self.local_iter % 10 == 0:
                ...
                # Messaggio informativo invece di warning
                if self.local_iter % 20 == 0:  # Meno spam nei log
                    print(f"INFO: Skipping contrastive loss at iter {self.local_iter} - "
                        f"Features not ready yet")
