# Modificato per gestire contrastive learning by Gionni 
...
class DACS(UDADecoratorFusion):
    ...
    def __init__(self, **cfg):
        ...
        self.enable_contrastive = cfg.get('enable_contrastive', False) # Se trova la chiave: Usa il valore associato, Se NON trova la chiave: Usa il valore di default False. Settato in: Nel tuo file di configurazione cs2dsec_image+events_together_b2.py
    ...
    def forward_train(self, **kwargs):
        ...
        # Parte aggiunta per gestire apprendimento contrastivo: VERSIONE DEFINITIVA
        if self.enable_contrastive:
            # DEBUG INIZIALE
            if self.local_iter % 20 == 0:
                print(f"[DEBUG] 🎯 === CONTRASTIVE DEBUG Iter {self.local_iter} ===")
                print(f"[DEBUG] 📊 enable_contrastive: {self.enable_contrastive}")
                print(f"[DEBUG] 📦 Inputs keys: {list(inputs.keys()) if inputs else 'None'}")
            ...
            # DEBUG FEATURES
            if self.local_iter % 20 == 0:
                print(f"[DEBUG] 📋 Pred dict keys: {list(pred_dict.keys())}")
                print(f"[DEBUG] 🎭 Contrastive features available: {'contrastive_features' in pred_dict}")
                
                if 'contrastive_features' in pred_dict:
                    print(f"[DEBUG] 🔑 Contrastive features keys: {list(contrastive_features.keys())}")
                    
                    for key, feat in contrastive_features.items():
                        status = "✅" if feat is not None else "❌"
                        shape = feat.shape if feat is not None else "None"
                        print(f"[DEBUG]    {status} {key}: {shape}")
            
            total_contrastive_loss = 0.0
            contrastive_count = 0
            
            # 1. CONTRASTIVE IMAGE-IMAGE (cross-domain giorno-notte) --> Sempre attivo quando le image features sono disponibili
            if (contrastive_features.get('image') is not None and
                contrastive_features['image'] is not None):
                
                anchor_img = contrastive_features['image']
                positive_img = contrastive_features['image']  # Self-supervised
                negative_img = anchor_img[torch.randperm(anchor_img.size(0))]
                
                # DEBUG IMAGE-IMAGE
                if self.local_iter % 20 == 0:
                    print(f"[DEBUG] 🖼️  Image-Image Contrastive:")
                    print(f"[DEBUG]    📐 Anchor shape: {anchor_img.shape}")
                    print(f"[DEBUG]    📐 Positive shape: {positive_img.shape}")
                    print(f"[DEBUG]    📐 Negative shape: {negative_img.shape}")
                
                loss_img = self.contrastive_loss(anchor_img, positive_img, negative_img)
                total_contrastive_loss += loss_img
                contrastive_count += 1
                
                if self.local_iter % 20 == 0:
                    similarity = F.cosine_similarity(anchor_img, positive_img).mean()
                    print(f"[DEBUG]    📈 Similarity: {similarity:.4f}")
                    print(f"[DEBUG]    💔 Loss: {loss_img.item():.4f}")
            
            # 2. CONTRASTIVE IMAGE-EVENTS (cross-modalità RGB-Events) --> Attivo solo quando entrambe le modalità sono disponibili
            if (contrastive_features.get('image') is not None and
                contrastive_features.get('events') is not None and
                contrastive_features['image'] is not None and
                contrastive_features['events'] is not None and
                contrastive_features['image'].shape[0] == contrastive_features['events'].shape[0]):
                
                anchor_img = contrastive_features['image']
                positive_events = contrastive_features['events']
                negative_events = positive_events[torch.randperm(positive_events.size(0))]
                
                # DEBUG IMAGE-EVENTS
                if self.local_iter % 20 == 0:
                    print(f"[DEBUG] 📡 Image-Events Contrastive:")
                    print(f"[DEBUG]    📐 Anchor image shape: {anchor_img.shape}")
                    print(f"[DEBUG]    📐 Positive events shape: {positive_events.shape}")
                    print(f"[DEBUG]    📐 Negative events shape: {negative_events.shape}")
                
                loss_cross_modal = self.contrastive_loss(anchor_img, positive_events, negative_events)
                total_contrastive_loss += loss_cross_modal
                contrastive_count += 1
                
                if self.local_iter % 20 == 0:
                    similarity = F.cosine_similarity(anchor_img, positive_events).mean()
                    print(f"[DEBUG]    📈 Cross-modal similarity: {similarity:.4f}")
                    print(f"[DEBUG]    💔 Cross-modal loss: {loss_cross_modal.item():.4f}")
            
            # 3. Calcola loss media --> Media delle due loss contrastive
            if contrastive_count > 0:
                avg_contrastive_loss = total_contrastive_loss / contrastive_count
                avg_contrastive_loss = self.contrastive_lambda * avg_contrastive_loss
                
                log_vars['loss_contrastive'] = avg_contrastive_loss.item()
                avg_contrastive_loss.backward()
                
                # DEBUG FINALE
                if self.local_iter % 20 == 0:
                    print(f"[DEBUG] 🧮 Total Contrastive Loss: {avg_contrastive_loss.item():.6f}")
                    print(f"[DEBUG] 🔢 Active modalities: {contrastive_count}")
                    print(f"[DEBUG] 📊 Loss breakdown: Image-Image + Image-Events = {total_contrastive_loss.item():.6f}")
                    print(f"[DEBUG] 🎉 Contrastive learning ACTIVE!")
                    print(f"[DEBUG] {'=' * 60}")
            
            else:
                # DEBUG NO CONTRASTIVE
                if self.local_iter % 20 == 0:
                    print(f"[DEBUG] ⚠️  No contrastive modalities available")
                    print(f"[DEBUG]    Possible reasons:")
                    print(f"[DEBUG]    - Image features: {contrastive_features.get('image') is not None}")
                    print(f"[DEBUG]    - Events features: {contrastive_features.get('events') is not None}")
                    if (contrastive_features.get('image') is not None and 
                        contrastive_features.get('events') is not None):
                        print(f"[DEBUG]    - Shapes match: {contrastive_features['image'].shape[0] == contrastive_features['events'].shape[0]}")
                    print(f"[DEBUG] {'=' * 60}")
