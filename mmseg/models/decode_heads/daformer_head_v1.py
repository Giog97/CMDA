...
class DAFormerHeadFusion(BaseDecodeHeadFusion):
    ...
    def __init__(self, **kwargs):
        ...
        # Nel costruttore di DAFormerHeadFusion --> aggiunta Gionni 4.9.2025 da verificare 
        #AGGIUNGI QUESTA PARTE DOPO TUTTE LE ALTRE INIZIALIZZAZIONI
        # Projection head per features contrastive
        self.proj_dim = 256
        self.projection_head = nn.Linear(self.channels, self.proj_dim)
    ...
    # AGGIUNGI QUESTO METODO DOPO forward_fusion E PRIMA DI forward
    def get_contrastive_features(self, feature_map):
        """
        Estrae features per contrastive learning - VERSIONE CORRETTA
        """
        if feature_map is None:
            print("[DEBUG] get_contrastive_features: feature_map is None")
            return None
        
        # 1. Global average pooling
        features = F.adaptive_avg_pool2d(feature_map, (1, 1))  # [B, C, 1, 1]
        # 2. Flatten to [B, C]
        features = features.view(features.size(0), -1)
        print(f"[DEBUG] Before projection: {features.shape}")
        # 3. Project to lower dimension
        features = self.projection_head(features)  # [B, proj_dim]
        # 4. Normalizzazione L2
        features = F.normalize(features, p=2, dim=1)
        print(f"[DEBUG] After projection: {features.shape}")
    ...
    def forward(self, inputs, size=None, cfg=None):
        ...
        # DEBUG: verifica che le features non siano None
        if cfg and cfg.get('local_iter', 0) % 10 == 0:
            print(f"Image feat: {image_feat is not None}")
            print(f"Events feat: {events_feat is not None}")
            print(f"Inputs keys: {list(inputs.keys()) if inputs else None}")
        
        # Estrai features per contrastive learning
        contrastive_features = {
            'image': self.get_contrastive_features(image_feat),
            'events': self.get_contrastive_features(events_feat),
            'fusion': self.get_contrastive_features(fusion_feat)
        }
        ...
        return {
            ...
            'contrastive_features': contrastive_features  # AGGIUNGI QUESTA LINEA per avere contrastive_features
        }
