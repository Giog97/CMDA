# Moidifcato by MaryGio
# Qui c'è la strategia su come vengono prese le immagini
import os
...
class DSECDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'events_vg' in self.outputs or 'negative_events' in self.outputs:
            ...
            # Leggi il file di indice UNA VOLTA sola all'inizio della funzione
            images_to_events_index_path = image_path.split('left/rectified')[0] + 'images_to_events_index.txt'
            try:
                with open(images_to_events_index_path, 'r') as f:
                    events_indices = [int(line.strip()) for line in f if line.strip()]
                
                # DEBUG
                print(f"[DEBUG] Total images in index: {len(events_indices)}")
                print(f"[DEBUG] Current image index: {now_image_index}")
                
                events_finish_index = events_indices[now_image_index]
                print(f"[DEBUG] Events finish index: {events_finish_index}")
                
            except Exception as e:
                print(f"❌ Error reading events index file {images_to_events_index_path}: {e}")
                events_finish_index = now_image_index * 10000

            # Ora calcola events_start_index usando events_indices
            if self.events_num != -1:
                events_start_index = events_finish_index - self.events_num + 1
            else:
                # ✅ USA events_indices invece di images_to_events_index
                start_idx = max(0, now_image_index - self.image_change_range)
                events_start_index = events_indices[start_idx]

            print(f"[DEBUG] Image index: {now_image_index}")
            print(f"[DEBUG] Events finish index: {events_finish_index}")
            print(f"[DEBUG] Events start index: {events_start_index}")
            print(f"[DEBUG] Events num: {self.events_num}")
            print(f"[DEBUG] Image change range: {self.image_change_range}")
            ...
            if 'negative_events' in self.outputs:
                neg_idx = idx
                attempt_count = 0
                max_attempts = 20  # Aumenta i tentativi
                
                # Estrai sequenza corrente
                current_sequence = image_path.split('/')[-5]
                
                while attempt_count < max_attempts:
                    neg_idx = random.randint(0, len(self) - 1)
                    
                    # Controllo PRINCIPALE: deve essere un indice diverso
                    if neg_idx == idx:
                        attempt_count += 1
                        continue
                        
                    neg_image_path = self.dataset_txt[neg_idx][0]
                    neg_sequence = neg_image_path.split('/')[-5]
                    
                    # DEBUG: stampa ogni 50 campioni
                    if idx % 50 == 0:
                        print(f"Sample {idx}: Positive={image_path}")
                        print(f"Sample {idx}: Negative={neg_image_path}")
                        print(f"Same index? {neg_idx == idx}, Same sequence? {current_sequence == neg_sequence}")
                    
                    # ACCETTA SIA STESSA SEQUENZA CHE SEQUENZE DIVERSE
                    # L'IMPORTANTE È CHE L'INDICE SIA DIVERSO
                    if neg_idx != idx:
                        break
                        
                    attempt_count += 1
                
                # DEBUG: stampa il risultato finale
                print(f"DEBUG: Positive idx: {idx}, Negative idx: {neg_idx}, Same index: {neg_idx == idx}")
                
                # FALLBACK: se non troviamo un indice diverso, usiamo una strategia di fallback
                if neg_idx == idx:
                    print("WARNING: Could not find different negative sample, using strategic fallback")
                    
                    # Strategia 1: prova l'indice successivo
                    neg_idx = (idx + 1) % len(self)
                    if neg_idx == idx:
                        # Strategia 2: prova l'indice precedente
                        neg_idx = (idx - 1) % len(self)
                    if neg_idx == idx:
                        # Strategia 3: usa un indice fisso diverso
                        neg_idx = 0 if idx != 0 else 1
                        
                    print(f"Using fallback negative idx: {neg_idx}")
                
                # CARICAMENTO degli eventi negativi
                neg_image_path = self.dataset_txt[neg_idx][0]
                ...
                try:
                    ...
                    # APPLICA DATA AUGMENTATION (stessa del positivo)
                    if 'label' not in self.outputs:
                    ...
                    output['negative_events'] = negative_events_vg
                    neg_events_h5.close()
                    
                except Exception as e:
                    print(f"ERROR loading negative events: {e}")
                    # Fallback: usa eventi positivi come negativi (meglio che nulla)
                    print("Using positive events as negative fallback")
                    output['negative_events'] = output['events_vg'].clone() if 'events_vg' in output else None
        # Aggiunto Per DEBUG
        # Nel __getitem__, aggiungi:
        if 'events_vg' in self.outputs:
            try:
                events_vg = self.get_events_vg_from_file(events_h5, events_finish_index, events_start_index, image_path)
                print(f"Loaded events for {image_path}: {events_vg is not None}")
            except Exception as e:
                print(f"ERROR loading events for {image_path}: {e}")
                events_vg = None
    ...
    def get_events_vg_from_file(self, events_h5, events_finish_index, events_start_index, image_path):
        try:
            # DEBUG: verifica gli indici
            total_events = events_h5['events/t'].shape[0]
            print(f"[DEBUG] Indici: start={events_start_index}, end={events_finish_index}, total={total_events}")
            
            # CONTROLLO DI SICUREZZA
            if events_start_index < 0:
                events_start_index = 0
                print(f"⚠️  start_index < 0, corretto a 0")
            if events_finish_index >= total_events:
                events_finish_index = total_events - 1
                print(f"⚠️  finish_index >= total, corretto a {events_finish_index}")
            if events_start_index > events_finish_index:
                print(f"⚠️  Indici invertiti per {image_path}, scambio")
                events_start_index, events_finish_index = events_finish_index, events_start_index
            
            # Solo se ci sono eventi nel range
            if events_finish_index - events_start_index <= 0:
                print(f"⚠️  Nessun evento nel range per {image_path}")
                return torch.zeros((self.events_bins, self.events_height, self.events_width))
            
            # Solo se supera tutti i controlli            
            events_t = np.asarray(events_h5['events/t'][events_start_index:events_finish_index+1])
            events_x = np.asarray(events_h5['events/x'][events_start_index:events_finish_index+1])
            events_y = np.asarray(events_h5['events/y'][events_start_index:events_finish_index+1])
            events_p = np.asarray(events_h5['events/p'][events_start_index:events_finish_index+1])
            ...
            print(f"❌ Errore in get_events_vg_from_file: {e}")
            return torch.zeros((self.events_bins, self.events_height, self.events_width))
